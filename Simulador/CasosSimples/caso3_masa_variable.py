...
# Simulaciones
for dt in dt_values:
    tiempos, sim = simular_dinamica(estado, t_max, dt, Integrador_oficial, der_gravedad_empuje_masa_var)
    pos_sim, vel_sim = zip(*[(s[0], s[1]) for s in sim])
    
    pos_analitica, vel_analitica = zip(*[sol_analitica_gravedad_empuje_masa_var(z0, v0, t, m0, beta, F0) for t in tiempos])
...
for label, integrador in integradores.items():
    tiempos_sim, sim = simular_dinamica(estado, t_max, dt, integrador, der_gravedad_empuje_masa_var)
