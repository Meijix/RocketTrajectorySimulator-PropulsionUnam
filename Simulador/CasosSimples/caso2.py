...
###Como aumento aqui para que sea 2d?
...
for metodo in metodos_adaptativos:
    ...
    for t, x,y, vx, vy in zip(sol.t, sol.y[0], sol.y[1], sol.y[2], sol.y[3]):
        #que es z_a y v_a
        # es la solucion analitica de este caso
        estado_a = sol_analitica(t, estado0)
        x_a, y_a, vx_a, vy_a = estado_a
        resultados.append([metodo+"_auto", np.nan, t, x, y, vx, vy, x_a, y_a, vx_a, vy_a])
...
for metodo in metodos_adaptativos:
    for dt in dt_vals:
        ...
        for t, x,y, vx, vy in zip(sol.t, sol.y[0], sol.y[1], sol.y[2], sol.y[3]):
            estado_a = sol_analitica(t, state=estado0)
            x_a, y_a, vx_a, vy_a = estado_a
            resultados.append([f"{metodo}_dt={dt}", dt, t, x,y, vx, vy, x_a, y_a, vx_a, vy_a])
...
for clase, nombre in metodos_fijos:
    for dt in dt_vals:
        ...
        while t < t_max:
            ...
            t += dt
            if nuevo_estado[1] < 0: break
            estado = nuevo_estado
            x, y, vx, vy = estado
            estado_a = sol_analitica(t, state=estado0)
            x_a, y_a, vx_a, vy_a = estado_a
            resultados.append([f"{nombre}_dt={dt}", dt, t, x,y, vx, vy, x_a, y_a, vx_a, vy_a])
